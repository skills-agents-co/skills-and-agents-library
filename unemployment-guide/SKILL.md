---
name: unemployment-guide
description: Walks a user through filing for unemployment insurance in their US state. Provides the official state agency name, application URL, phone number, eligibility requirements, step-by-step application instructions, required documents, weekly certification rules, waiting period, and common pitfalls that cause denials. Use this skill whenever the user mentions losing their job, getting laid off, getting fired, filing for unemployment, applying for UI benefits, "EDD" (California), "DUA", "DES", "DEW", a state workforce agency, or asks any variation of "how do I get unemployment" — even if they don't name the program precisely. Also triggers on "/unemployment", "I just got laid off", "lost my job", "file a UI claim", "unemployment benefits", "jobless claim", or pasting a termination letter and asking what to do next. Works for all 50 states plus DC.
---

# Unemployment Guide

Help the user file for unemployment insurance (UI) benefits in their state. The goal is to get them from "I lost my job" to "my claim is filed correctly" with no missed steps that would delay or deny their benefits.

## Important note about accuracy

The state-specific data in this skill (URLs, phone numbers, waiting periods, max weeks) is accurate as of the last update but state legislatures change unemployment law regularly. If anything looks dead or off, fall back to [CareerOneStop's state finder](https://www.careeronestop.org/LocalHelp/UnemploymentBenefits/find-unemployment-benefits.aspx), which the US Department of Labor keeps current. This skill is procedural guidance, not legal advice — for appeals or contested separations, point the user to legal aid or an employment attorney.

## Step 1 — Get the state

If the user hasn't named a state, ask: "Which state did you work in?" Note: file in the state where you **worked**, not where you live.

**Multi-state workers (worked in two or more states in the last 18 months):** They may have a "combined wage claim" and have a choice of which state to file in. The standard US Department of Labor guidance is to contact the state where they **most recently worked** — that state will explain combined-wage options and route the claim correctly. Do **not** default to "file where you live." For someone who works remotely or recently relocated, the resident state may not be one of the states where they earned wages, and filing there can send them to the wrong agency and delay benefits.

## Step 2 — Pull the state-specific details

Find the user's state in the "State data" section at the bottom of this skill. That entry has:
- Official agency name
- Application URL (file online)
- Claimant phone number
- Waiting period (1 wk unpaid, or none)
- State-specific notes worth flagging

If the user's state isn't in the data below, or if a URL looks stale, fall back to [CareerOneStop](https://www.careeronestop.org/LocalHelp/UnemploymentBenefits/find-unemployment-benefits.aspx).

## Step 3 — Walk through the universal flow

Most states share the same shape. Present this as a numbered checklist tailored with the state's specifics from the data section.

**Before you apply, gather:**
- Social Security number
- Government-issued photo ID (driver's license or state ID number)
- Mailing address, phone, email
- Bank routing + account number (for direct deposit; otherwise they'll mail a debit card, which is slower)
- For every employer in the last 18 months:
  - Legal company name and address
  - Dates worked (start and end)
  - Reason for separation (laid off / fired / quit / still employed but reduced hours)
  - Gross wages earned
- If not a US citizen: Alien Registration Number and work authorization details
- If military in last 18 months: DD-214
- If federal employee: SF-8 and SF-50
- If they have dependents and the state pays a dependent allowance: dependents' SSNs and DOBs

**Apply:**
1. Go to the state's UI portal (URL from the data section).
2. Create an account. Save the username/password somewhere safe — they'll need it weekly.
3. Complete the initial claim. Be honest and precise about the separation reason. "Laid off due to lack of work" is the cleanest path; "fired" or "quit" triggers extra fact-finding and is the most common cause of delay.
4. Set up direct deposit during signup if available.
5. Submit and write down the confirmation number.

**After you apply:**
1. **Register for work search** on the state's job board if required (most states require this within 7 days — check the state notes). Missing this is a top reason for denial.
2. **File your weekly (or biweekly) certification.** This is the step people forget. Cadence varies — most states are weekly, some (CA, IL, TX, AK) are biweekly. Check the state notes. Even if your claim is still "pending," you must certify every period from week one or you forfeit those weeks permanently. Most states won't let you back-certify.
3. **Log work-search activities.** Most states require 3–5 employer contacts per week and require you to keep a log (some require you to enter them in the portal). Keep records for at least a year — audits happen.
4. **Watch for a determination letter** in the mail or portal inbox. If denied, you typically have 10–30 days to appeal — appeal immediately even if you think it's wrong; you can build the case later.
5. **First payment usually arrives 2–3 weeks after filing**, longer if there's a waiting week or any fact-finding.

## Step 4 — Flag the common pitfalls

Always mention these — they cause the majority of denials and delays:

- **Quitting vs. laid off:** If you quit, you generally won't qualify unless you had "good cause" (unsafe conditions, harassment, medical necessity, spouse relocation in some states). Don't say "I quit" if you were pressured to resign — describe what actually happened.
- **Fired for misconduct:** "Misconduct" is a narrow legal term, not just "the boss was unhappy." Performance issues usually don't count as misconduct. Apply anyway and let the state decide.
- **Severance:** Some states offset benefits by severance, some don't. Report it accurately.
- **1099 / gig work:** Traditional UI doesn't cover independent contractors. If they think they were misclassified (paid as 1099 but treated like an employee), they can still apply and request a misclassification review.
- **Not certifying:** Repeat this — it's the #1 way people lose money they were entitled to.
- **Refusing "suitable work":** Once collecting, turning down a reasonable job offer can end benefits.
- **Working while collecting:** Always report any earnings during a certification week, even cash or part-time. Unreported earnings = fraud charge + repayment + penalty.
- **Identity verification:** Many states use ID.me or a similar verifier. Have your ID and a selfie-capable phone ready; this step blocks payment until completed.

## Step 5 — Give them the phone number and tell them when to use it

Pull the phone number from the data section. Tell them:
- Call **only** if the portal is broken, they can't verify identity online, or they get a determination they want to appeal.
- Best times to call are typically Tuesday–Thursday, mid-morning or mid-afternoon. Mondays are the worst.
- Have their SSN, claim ID, and a pen ready before calling.

## Output shape

Give the answer as a clean, scannable response with these sections in order:
1. **Your state's agency** (name, URL, phone) — one line each
2. **What to gather before you apply** — checklist
3. **How to apply** — numbered steps
4. **After you apply** — weekly/biweekly certification + work search
5. **Watch out for** — top 3–4 pitfalls most relevant to their situation (if they mentioned being fired, lead with the misconduct one; if 1099, lead with that)
6. **If something goes wrong** — phone number + when to call

Keep it practical. Don't lecture about the program's history. The user is stressed and needs to act.

## Tone

This is a high-stakes, often emotional moment. Be direct, calm, and concrete. Don't pad with reassurance, but don't be cold either. Treat them like a competent adult who needs accurate information fast.

---

# State data

For each state: official agency, file-a-claim URL, claimant phone, waiting period, and notes. URLs and phones change occasionally — if a link looks dead, fall back to [CareerOneStop](https://www.careeronestop.org/LocalHelp/UnemploymentBenefits/find-unemployment-benefits.aspx).

Waiting period: "1 wk" means the first eligible week is unpaid. "None" means benefits start week 1.

## Alabama
- **Agency:** Alabama Department of Labor
- **File:** https://labor.alabama.gov/uc/
- **Phone:** 1-800-361-4524
- **Waiting period:** 1 wk
- **Notes:** Must register for work at AlabamaWorks.com. Work search: 3 contacts/week.

## Alaska
- **Agency:** Alaska Department of Labor and Workforce Development
- **File:** https://my.alaska.gov/ (then UI Online)
- **Phone:** 1-888-252-2557
- **Waiting period:** 1 wk
- **Notes:** Files biweekly, not weekly. Dependent allowance available ($24/dependent/week, up to 3).

## Arizona
- **Agency:** Arizona Department of Economic Security (DES)
- **File:** https://des.az.gov/services/employment/unemployment-individual
- **Phone:** 1-877-600-2722
- **Waiting period:** 1 wk
- **Notes:** Max benefit is one of the lowest in the country (~$320/wk as of recent updates). Work search: 4 contacts/week.

## Arkansas
- **Agency:** Arkansas Division of Workforce Services
- **File:** https://www.ezarc.adws.arkansas.gov/
- **Phone:** 1-844-908-2178
- **Waiting period:** 1 wk
- **Notes:** Must register at Arkansas JobLink within 7 days.

## California
- **Agency:** Employment Development Department (EDD)
- **File:** https://edd.ca.gov/en/Unemployment/
- **Phone:** 1-800-300-5616
- **Waiting period:** 1 wk
- **Notes:** Use UI Online via myEDD account. ID.me identity verification required for most claimants. Phone wait times are notoriously long — exhaust the portal first. Certify every two weeks. Has the highest max weekly benefit on the West Coast but processing delays are common — file the day you're separated.

## Colorado
- **Agency:** Colorado Department of Labor and Employment
- **File:** https://myui.coworkforce.com/
- **Phone:** 303-318-9000 (Denver-metro) / 1-800-388-5515
- **Waiting period:** None (Colorado pays from week 1)
- **Notes:** ID.me verification required. Must register at Connecting Colorado.

## Connecticut
- **Agency:** Connecticut Department of Labor
- **File:** https://filectui.com/
- **Phone:** 203-941-6868
- **Waiting period:** None
- **Notes:** New "ReEmployCT" system replaced the legacy one in 2022. Dependent allowance ($15/child/week).

## Delaware
- **Agency:** Delaware Division of Unemployment Insurance
- **File:** https://ui.delawareworks.com/
- **Phone:** 302-761-8446
- **Waiting period:** None
- **Notes:** Work search: 2 contacts/week (lower than most states).

## District of Columbia
- **Agency:** DC Department of Employment Services (DOES)
- **File:** https://does.dc.gov/service/unemployment-compensation-claimants
- **Phone:** 202-724-7000
- **Waiting period:** 1 wk
- **Notes:** Must register at DC Networks within 7 days.

## Florida
- **Agency:** Florida Department of Commerce / Reemployment Assistance (formerly DEO)
- **File:** https://connect.myflorida.com/
- **Phone:** 1-833-352-7759
- **Waiting period:** 1 wk
- **Notes:** **Work registration with Employ Florida is required for the claim to be processed.** Florida integrates work registration into the claim flow — you can submit the initial claim first, but benefits will not be paid until registration is complete, so complete both promptly (same-day is best). Max benefit is only 12 weeks (lowest in the country) and ~$275/wk. Work search: 5 contacts/week. Quirky CONNECT portal — keep screenshots.

## Georgia
- **Agency:** Georgia Department of Labor
- **File:** https://dol.georgia.gov/individuals/unemployment-services
- **Phone:** 404-232-3001 (metro) / 1-877-709-8185
- **Waiting period:** 1 wk
- **Notes:** Many employers in GA file the initial claim on behalf of laid-off workers ("employer-filed claim"). Ask HR if they're doing this before you file yourself — duplicate claims slow things down.

## Hawaii
- **Agency:** Hawaii Department of Labor and Industrial Relations
- **File:** https://huiclaims.hawaii.gov/
- **Phone:** Oahu 808-586-8970, Hilo 808-974-4086, Maui 808-984-8400, Kauai 808-274-3043
- **Waiting period:** 1 wk
- **Notes:** Must complete in-person or virtual orientation in some cases.

## Idaho
- **Agency:** Idaho Department of Labor
- **File:** https://www.labor.idaho.gov/dnn/Unemployment-Benefits
- **Phone:** 208-332-8942
- **Waiting period:** 1 wk
- **Notes:** Work search: 2 contacts/week minimum.

## Illinois
- **Agency:** Illinois Department of Employment Security (IDES)
- **File:** https://ides.illinois.gov/
- **Phone:** 1-800-244-5631
- **Waiting period:** 1 wk
- **Notes:** ID.me verification. Dependent allowance available. Certify every two weeks.

## Indiana
- **Agency:** Indiana Department of Workforce Development
- **File:** https://uplink.in.gov/
- **Phone:** 1-800-891-6499
- **Waiting period:** 1 wk
- **Notes:** Must register on Indiana Career Connect.

## Iowa
- **Agency:** Iowa Workforce Development
- **File:** https://www.iowaworkforcedevelopment.gov/file-claim-unemployment-insurance-benefits
- **Phone:** 1-866-239-0843
- **Waiting period:** None
- **Notes:** Reduced max weeks (16) as of recent legislation. Aggressive work-search requirements.

## Kansas
- **Agency:** Kansas Department of Labor
- **File:** https://www.getkansasbenefits.gov/
- **Phone:** 785-575-1460
- **Waiting period:** 1 wk
- **Notes:** Must register at KANSASWORKS.

## Kentucky
- **Agency:** Kentucky Career Center / Office of Unemployment Insurance
- **File:** https://kcc.ky.gov/career/Pages/Unemployment-Services.aspx
- **Phone:** 502-875-0442
- **Waiting period:** 1 wk
- **Notes:** ID.me verification.

## Louisiana
- **Agency:** Louisiana Workforce Commission
- **File:** https://www.louisianaworks.net/hire/
- **Phone:** 1-866-783-5567
- **Waiting period:** 1 wk
- **Notes:** Work search: 3 contacts/week.

## Maine
- **Agency:** Maine Department of Labor
- **File:** https://reemployme.maine.gov/
- **Phone:** 1-800-593-7660
- **Waiting period:** None
- **Notes:** Dependent allowance available.

## Maryland
- **Agency:** Maryland Department of Labor / BEACON
- **File:** https://www.labor.maryland.gov/employment/unemployment.shtml (then BEACON portal)
- **Phone:** 667-207-6520
- **Waiting period:** None
- **Notes:** BEACON portal — must register at Maryland Workforce Exchange.

## Massachusetts
- **Agency:** Department of Unemployment Assistance (DUA)
- **File:** https://www.mass.gov/unemployment-insurance-ui-online
- **Phone:** 877-626-6800
- **Waiting period:** None
- **Notes:** Dependent allowance ($25/child/week). Among the most generous max benefits in the US.

## Michigan
- **Agency:** Michigan Unemployment Insurance Agency (UIA)
- **File:** https://miwam.unemployment.state.mi.us/
- **Phone:** 1-866-500-0017
- **Waiting period:** 1 wk
- **Notes:** MILogin + MiWAM accounts. ID verification can be slow.

## Minnesota
- **Agency:** Minnesota Unemployment Insurance
- **File:** https://www.uimn.org/
- **Phone:** 651-296-3644 (metro) / 1-877-898-9090
- **Waiting period:** None
- **Notes:** Files weekly; one of the cleaner state portals.

## Mississippi
- **Agency:** Mississippi Department of Employment Security
- **File:** https://mdes.ms.gov/
- **Phone:** 601-493-9427
- **Waiting period:** 1 wk
- **Notes:** Max 26 weeks. Work search: 3 contacts/week.

## Missouri
- **Agency:** Missouri Department of Labor / Division of Employment Security
- **File:** https://uinteract.labor.mo.gov/
- **Phone:** 1-800-320-2519
- **Waiting period:** 1 wk
- **Notes:** Max weeks recently reduced based on unemployment rate (sliding scale, often 13–20 weeks).

## Montana
- **Agency:** Montana Department of Labor and Industry
- **File:** https://uid.dli.mt.gov/
- **Phone:** 406-444-2545
- **Waiting period:** 1 wk
- **Notes:** Must register on MontanaWorks.

## Nebraska
- **Agency:** Nebraska Department of Labor / NEworks
- **File:** https://neworks.nebraska.gov/
- **Phone:** 402-458-2500
- **Waiting period:** 1 wk
- **Notes:** Same portal for filing and work search.

## Nevada
- **Agency:** Nevada Department of Employment, Training and Rehabilitation (DETR)
- **File:** https://ui.unemployment.nv.gov/css.html
- **Phone:** Northern 775-684-0350, Southern 702-486-0350, Rural 1-888-890-8211
- **Waiting period:** 1 wk
- **Notes:** Identity verification often delays first payment.

## New Hampshire
- **Agency:** NH Employment Security
- **File:** https://www.nhes.nh.gov/
- **Phone:** 603-271-7700
- **Waiting period:** None
- **Notes:** Must report to NH Works for orientation in some cases.

## New Jersey
- **Agency:** NJ Department of Labor and Workforce Development
- **File:** https://myunemployment.nj.gov/
- **Phone:** North 201-601-4100, Central 732-761-2020, South 856-507-2340
- **Waiting period:** 1 wk (refundable if you collect through your full benefit period)
- **Notes:** ID.me verification. Dependent allowance.

## New Mexico
- **Agency:** NM Department of Workforce Solutions
- **File:** https://www.dws.state.nm.us/
- **Phone:** 1-877-664-6984
- **Waiting period:** 1 wk
- **Notes:** Must register at NM Workforce Connection.

## New York
- **Agency:** NY State Department of Labor
- **File:** https://applications.labor.ny.gov/IndividualReg/
- **Phone:** 1-888-209-8124
- **Waiting period:** 1 wk
- **Notes:** ID.me verification. Certify weekly via web or Tel-Service (1-888-581-5812). NY.gov ID required first.

## North Carolina
- **Agency:** NC Division of Employment Security
- **File:** https://des.nc.gov/
- **Phone:** 1-888-737-0259
- **Waiting period:** 1 wk
- **Notes:** Max weeks is among the lowest (12–20 weeks based on unemployment rate). Strict work-search: 3 contacts/week, must log in NCWorks.

## North Dakota
- **Agency:** Job Service North Dakota
- **File:** https://www.jobsnd.com/unemployment-individuals
- **Phone:** 701-328-4995
- **Waiting period:** 1 wk
- **Notes:** Must register at Job Service ND.

## Ohio
- **Agency:** Ohio Department of Job and Family Services (ODJFS)
- **File:** https://unemployment.ohio.gov/
- **Phone:** 1-877-644-6562
- **Waiting period:** 1 wk
- **Notes:** Dependent allowance. ID verification often via ID.me.

## Oklahoma
- **Agency:** Oklahoma Employment Security Commission
- **File:** https://unemployment.state.ok.us/
- **Phone:** 405-525-1500
- **Waiting period:** 1 wk
- **Notes:** ID.me verification.

## Oregon
- **Agency:** Oregon Employment Department / Frances Online
- **File:** https://frances.oregon.gov/
- **Phone:** 1-877-345-3484
- **Waiting period:** 1 wk
- **Notes:** Frances Online replaced the legacy system in 2024. Also handles Paid Leave Oregon — make sure you're filing in the UI portal, not Paid Leave.

## Pennsylvania
- **Agency:** PA Department of Labor and Industry
- **File:** https://www.uc.pa.gov/
- **Phone:** 1-888-313-7284
- **Waiting period:** None
- **Notes:** Dependent allowance ($5/dependent/week, up to 2). Keystone ID required.

## Rhode Island
- **Agency:** RI Department of Labor and Training
- **File:** https://uionline.dlt.ri.gov/
- **Phone:** 401-415-6772
- **Waiting period:** None
- **Notes:** Dependent allowance.

## South Carolina
- **Agency:** SC Department of Employment and Workforce (DEW)
- **File:** https://dew.sc.gov/
- **Phone:** 1-866-831-1724
- **Waiting period:** 1 wk
- **Notes:** Must register at SC Works Online Services. Work search: 2 contacts/week.

## South Dakota
- **Agency:** SD Department of Labor and Regulation
- **File:** https://dlr.sd.gov/ra/individuals/default.aspx
- **Phone:** 605-626-2452
- **Waiting period:** 1 wk
- **Notes:** Must register at SDWORKS.

## Tennessee
- **Agency:** TN Department of Labor and Workforce Development
- **File:** https://www.jobs4tn.gov/
- **Phone:** 1-844-224-5818
- **Waiting period:** 1 wk
- **Notes:** Strict work-search: 3 contacts/week, must be logged in Jobs4TN.

## Texas
- **Agency:** Texas Workforce Commission (TWC)
- **File:** https://www.twc.texas.gov/programs/unemployment-benefits
- **Phone:** 1-800-939-6631 (Tele-Center)
- **Waiting period:** 1 wk
- **Notes:** Register on WorkInTexas within 3 business days — Texas is strict about this. Apply via "Unemployment Benefits Services" portal. Files biweekly. Phone Tele-Center hours are M–F 7am–6pm.

## Utah
- **Agency:** Utah Department of Workforce Services
- **File:** https://jobs.utah.gov/ui/home
- **Phone:** Salt Lake/South Davis 801-526-4400, Weber/North Davis 801-612-0877, statewide 1-888-848-0688
- **Waiting period:** 1 wk
- **Notes:** Aggressive work search (4/week) and audits.

## Vermont
- **Agency:** Vermont Department of Labor
- **File:** https://labor.vermont.gov/unemployment-insurance
- **Phone:** 1-877-214-3330
- **Waiting period:** 1 wk
- **Notes:** Dependent allowance.

## Virginia
- **Agency:** Virginia Employment Commission (VEC)
- **File:** https://www.vec.virginia.gov/unemployed
- **Phone:** 1-866-832-2363
- **Waiting period:** 1 wk
- **Notes:** ID.me verification.

## Washington
- **Agency:** WA Employment Security Department (ESD)
- **File:** https://esd.wa.gov/unemployment
- **Phone:** 1-800-318-6022
- **Waiting period:** 1 wk
- **Notes:** SecureAccess Washington (SAW) account required. Work search: 3 job-search activities/week, logged in eServices.

## West Virginia
- **Agency:** WorkForce West Virginia
- **File:** https://workforcewv.org/
- **Phone:** 1-800-379-1032
- **Waiting period:** 1 wk
- **Notes:** Must register at WorkForce WV.

## Wisconsin
- **Agency:** WI Department of Workforce Development
- **File:** https://my.unemployment.wisconsin.gov/
- **Phone:** 414-435-7069 (Milwaukee), 608-232-0824 (Madison), 1-844-910-3661 elsewhere
- **Waiting period:** 1 wk
- **Notes:** Must register at Job Center of Wisconsin within 14 days. Work search: 4 actions/week.

## Wyoming
- **Agency:** WY Department of Workforce Services
- **File:** https://wyui.wyo.gov/benefits/
- **Phone:** 307-473-3789
- **Waiting period:** 1 wk
- **Notes:** Must register at Wyoming at Work.

---

## Eval Contract

### Spec

A correct response names the user's state agency, application URL, and claimant phone number for the state where they worked (not where they live), then gives them an ordered path: what to gather, how to file, how to certify weekly or biweekly, and the pitfalls most relevant to their situation. The agency name, URL, and phone must match the state's official UI program. Filing-state guidance must be tied to the work state, multi-state workers must be pointed at the most-recent-work state, and certification cadence must match the named state. The response is action-oriented and scannable, not a history lesson.

### Rubric

Score each dimension 0 or 1, total out of 8. Run the hard-fail gate first.

**Hard-fail gate (check before scoring):** If the response gives a wrong state agency name, a wrong or fabricated application URL, or a wrong claimant phone number for the named state, the response is an automatic fail regardless of total score. Wrong official contact details send a stressed user to the wrong place and is the worst outcome this skill can produce.

| # | Dimension | Pass | Fail | Weight |
|---|-----------|------|------|--------|
| 1 | Correct state resolution | Files in the state where the user worked; asks for the state if not given | Defaults to residence state or guesses without asking | 1 |
| 2 | Agency/URL/phone present | All three official contact details given, one line each | Any of the three missing | 1 |
| 3 | Multi-state handling | If user worked in 2+ states, points them at the most-recent-work state and names combined-wage option | Defaults to "file where you live" or ignores multi-state | 1 |
| 4 | Document checklist | Lists the documents to gather before applying (SSN, ID, employer history, bank info) | No checklist, or omits employer-history requirement | 1 |
| 5 | Certification cadence correct | States weekly vs biweekly correctly for the named state and stresses certifying from week one | Wrong cadence, or omits the certify-every-week warning | 1 |
| 6 | Separation-reason guidance | Explains laid-off vs quit vs fired honestly and tells them to describe what actually happened | Tells the user to misrepresent the separation, or omits the topic | 1 |
| 7 | Relevant pitfalls surfaced | Leads with the pitfall matching the user's stated situation (misconduct if fired, misclassification if 1099) | Generic pitfall dump with no tailoring, or none | 1 |
| 8 | Scope discipline | Frames itself as procedural guidance, points contested cases to legal aid, no legal advice on the merits | Renders a legal verdict on eligibility or an appeal's outcome | 1 |

**Score to action:** 8/8 ship. 6 to 7 acceptable, note the gap. 4 to 5 borderline, flag for human review. 0 to 3 bad, root-cause. Any hard-fail gate trip is fail regardless of total.

### Self-Test

**Scenario A — "I got laid off in Texas, what do I do?"**
- The output MUST name Texas Workforce Commission (TWC) as the agency.
- The output MUST give the claimant phone number `1-800-939-6631`.
- The output MUST state that Texas certifies biweekly, not weekly.
- The output MUST tell the user to register on WorkInTexas within the required window.
- The output MUST NOT tell the user to file in a different state than Texas.

**Scenario B — "I worked in New Jersey but I live in Pennsylvania. I was fired. Where do I file?"**
- The output MUST direct the user to file in New Jersey (the work state), not Pennsylvania (the residence state).
- The output MUST address the "fired" separation reason and explain that misconduct is a narrow legal term and they should apply anyway.
- The output MUST NOT advise the user to claim they were laid off if they were not.
- The output MUST NOT state a fabricated phone number or URL for either state.

**Scenario C — "I drove for a rideshare app on a 1099 and they cut me off."**
- The output MUST explain that traditional UI generally does not cover independent contractors.
- The output MUST mention the misclassification-review path (apply anyway and request review if they believe they were misclassified).
- The output MUST NOT promise the user they will qualify.

### Version

1.0.0

**More from Uristocrat Studios:** see this skill in the [Skills & Agents catalog](https://skillsandagents.co/skills/unemployment-guide/).

