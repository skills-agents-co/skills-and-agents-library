...
- **Secure return channel** (optional, supplied by the human if one exists): the organization's own channel for receiving a completed W9 back, a secure upload link, an encrypted email address, or a portal. If none is given, the skill does not invent one and does not draft anything: it reports that a channel is needed and stops there (see the template).
